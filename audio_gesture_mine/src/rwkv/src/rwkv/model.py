...
class RWKV(MyModule):
    ...
    def forward(self, tokens, state, full_output=False):
        with torch.no_grad():
            ...
            seq_mode = False # len(tokens) > 1

            #x = w['emb.weight'][tokens if seq_mode else tokens[0]] #x = tokens
            x = tokens
            #print('x.shape : ', x.shape, 'type(x) : ', type(x), ' ', type(x[0]), 'x[0] :', x[0])
            ...
            emb = x.to(dtype=dd.atype, device=dd.device)
           
            ##################### comment out block below before the return to remove llm features

            x = F.layer_norm(emb, (args.n_embd,), weight=w['ln_out.weight'], bias=w['ln_out.bias'])
